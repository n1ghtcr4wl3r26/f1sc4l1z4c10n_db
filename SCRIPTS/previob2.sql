/* Formatted on 1-ago.-2017 22:28:47 (QP5 v5.126) */
SELECT   f1.ctl_cod_gestion gestion,
         ...
         DECODE (
             ...
             UPPER(   f1.ctl_nombres
                   ...
                   || f1.ctl_apmat))
             identidad_nombre,
         pkg_reporte.devuelve_fiscalizadores_reg (f1.ctl_control_id)
             fiscalizador,
         pkg_reporte.devuelve_fiscalizadores_acc (f1.ctl_control_id)
             fiscalizador_nuevo,
         pkg_reporte.devuelve_jefe_reg (f1.ctl_control_id) supervisor,
         pkg_reporte.devuelve_jefe_acc (f1.ctl_control_id) supervisor_nuevo,
         ...
         con.con_num_doc_con,
         TO_CHAR (con.con_fecha_doc_con, 'dd/mm/yyyy') con_fecha_doc_con,
         con.con_usuario,
         pkg_reporte.devuelve_info_conclusion (f1.ctl_control_id,
                                               con.con_tipo_doc_con)
             info_con,
         e1.est_estado estado_control,
         pkg_reporte.devuelve_ilicitos (f1.ctl_control_id) ilicitos,
         TO_CHAR (
             noti.not_fecha_notificacion + NVL (in1.inn_plazo_conclusion, 0),
             'dd/mm/yyyy')
             fecha_est_con,
 ...
 WHERE       sad_gen.key_year = prov.key_year(+)
         ...
         AND a1.est_fecsys BETWEEN TO_DATE ('01/01/2017', 'dd/mm/yyyy')
                               AND  TO_DATE ('01/08/2017', 'dd/mm/yyyy')
