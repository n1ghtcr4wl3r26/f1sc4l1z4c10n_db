/* Formatted on 01/08/2017 16:30:33 (QP5 v5.126) */
SELECT      sad_gen.key_cuo
         ...
         (SELECT   DISTINCT
                   DECODE (cty.cty_cod,
                           ...
                           'TN', 'TUNEZ',
                           UPPER (cty.cty_dsc))
            FROM   ops$asy.unctytab cty
           WHERE   itm.saditm_cty_origcod = cty.cty_cod(+)
                   AND cty.lst_ope(+) = 'U')
             pais_origen,
         NVL (
             (SELECT   DISTINCT
         ...
         || SUBSTR (sad_gen.sad_lop_cod, 1, 2)
             pto_pais_embarque                                     -- campo 27
                              ,
         itm.saditm_gross_mass total_peso_bruto               -- campo 6b (84)
                                               ,
         itm.saditm_net_mass total_peso_neto                  -- campo 6b (84)
                                            ,
         vim.sad_iitminv_valc "TOTAL VALOR FOB",             -- campo 22b (48)
         NVL (vim.sad_iitmefr_valc, 0) total_flete,
         NVL (vim.sad_iitmins_valc, 0) total_seguro,
         NVL (vim.sad_iitmotc_valc, 0) total_otros,
         NVL (ROUND (vim.sad_iitmcif_valn / vim.sad_iitminv_rat, 2), 0)
             cifusd,
         ...
         NVL (icda.saditm_tax_amount, 0) tot_trib
