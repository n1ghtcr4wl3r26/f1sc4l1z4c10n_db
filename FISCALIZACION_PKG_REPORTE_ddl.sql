...
/* Formatted on 31/05/2017 17:51:29 (QP5 v5.126) */
IS
    FUNCTION devuelve_tributos (prm_codigo IN VARCHAR2)
        RETURN cursortype
    IS
        ...
        OPEN ct FOR
              SELECT                                                       --1
                    t  .itm,
                       ...
                       ROUND ( (t.to_iva + t.iva_dt) / t.tc_ufvhoy, 2)
                           dt_iva,
                       ROUND ( (t.to_ice + t.ice_dt) / t.tc_ufvhoy, 2)
                           dt_ice,
                       ROUND ( (t.to_iehd + t.iehd_dt) / t.tc_ufvhoy, 2)
                           dt_iehd,
                       ROUND ( (t.to_icd + t.icd_dt) / t.tc_ufvhoy, 2)
                           dt_icd,
                       --60
                       ROUND (
                           (  t.to_ga
